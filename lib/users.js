...
module.exports = {
    ...
    methods: {
        ...
        async addUser({ email = null, password = null, locale = null }) {
            ...
            statement += "WITH u, ";
            statement += " CASE WHEN u.uid IS NULL THEN $uid ELSE u.uid END AS uid, ";
            statement += " CASE WHEN u.password IS NULL THEN $password ELSE u.password END AS password, ";
            statement += " CASE WHEN u.locale IS NULL THEN $locale ELSE u.locale END AS locale, ";
            statement += " CASE WHEN u.verified IS NULL THEN $verified ELSE u.verified END AS verified, ";
            statement += " CASE WHEN u.data IS NULL THEN $data ELSE u.data END AS data ";
            statement += "SET u.uid = uid, u.password = password, u.locale = locale, u.verified = verified, u.data = data ";
